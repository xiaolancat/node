...
// 添加友链
adminController.ahrefAdd = function(req,res){
    // 响应模版
    res.render('admin/ahrefAdd');     
}

// 插入友链数据
adminController.ahrefInsert = function(req,res){
    // 插入数据到数据库
    ahrefModel.create(req.body,function (err) {
        if(err){
            console.log('数据添加数据失败');
        }else{
            // res.send('数据插入成功');
            // 跳转到首页
            res.redirect('/admin/ahrefList');
        }
    })
}
...
// 友链列表
adminController.ahrefList = function (req,res) {
    ahrefModel.find().sort({order:1}).exec(function(err,data){
        if (err) {
            console.log('数据添加数据失败');
        } else {
            // 响应模版 分配数据
            res.render('admin/ahrefList',{datalist:data})
        }
    })
...
// 编辑友链的页面
adminController.ahrefEdit = function(req,res){
    // 查询要编辑的数据
    ahrefModel.find({_id:req.params._id},function(err,data){
        if(err){
           res.send('查询数据失败') 
        }else{
            // 响应模版
            res.render('admin/ahrefEdit',{data:data[0]});
        }
    })
}

// 修改友链数据
adminController.ahrefUpdate = function (req,res) {
    // 更新数据
    ahrefModel.update({_id:req.body._id},req.body,function(error){
        // 跳转到栏目列表
        res.redirect('/admin/ahrefList');
    })
}
...
// 删除友链数据
adminController.ahrefRemove = function (req,res) {
    // 数据库删除的操作 
    ahrefModel.remove({ _id: req.params._id},function (error) {
        if (error) {
            res.send(error)
        } else {
            // 跳转到友链列表
